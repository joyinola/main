...
def item(request,item_id):
    ...
    return render(request,'auctions/item.html', {'product':entry,'use':use,'bid':bid,'comment':commentt, 'active':active,
        'closed':close,'bidder':hightest_bidder})
...
def listin(request):
    ...
    if request.method=='POST':
        ...
        productt=product.objects.create(item=l,img=i,category=categoryy,price=p,description=d,user=request.user,is_active=True)
...
def al_watchlist(request):
    ...
    #return HttpResponse(everything)
